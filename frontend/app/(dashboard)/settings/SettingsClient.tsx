...
// Populated from Stripe once billing history wiring lands (Phase 7).
// Never show sample invoices — an empty list is the honest state.
...
export function SettingsClient({ userName, userEmail }: SettingsClientProps) {
  ...
  // Password change is not applicable with magic-link auth.
  // Kept as no-op to avoid removing the security tab UI.
  const handleChangePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    toast.info("Password management is not available — you sign in via magic link.");
  };
      ...
      <Tabs defaultValue="Account">
        ...
        <TabsContent value="Team Management">
          <div className="max-w-2xl space-y-5">
            <div className="glass-panel rounded-2xl p-8 flex flex-col items-center text-center gap-3">
        ...
        <TabsContent value="Billing & Subscription">
          <div className="space-y-6">
            ...
            <div className="border border-dashed border-border/50 rounded-2xl p-5 flex items-center gap-4 bg-white/1">
              ...
              <Button variant="outline" className="shrink-0 rounded-xl border-border/40 h-9 px-4 text-sm" asChild>
                <a href="mailto:founder@getfrugal.dev?subject=Corporate%20Plan%20Interest">
                  Register Interest
                </a>
              </Button>
            </div>
            ...
            <div className="glass-panel rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" disabled className="h-8 rounded-xl border-border/40 text-xs gap-1.5">
                ...
                <div className="divide-y divide-border">
                  {billingInvoices.map((inv) => (
                    <div key={inv.id} className="px-5 py-3.5 grid grid-cols-6 gap-3 items-center hover:bg-white/2 transition-colors">
                      ...
                      <span className="text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-md font-semibold w-fit">
                        {inv.status}
                      </span>
                    </div>
                  ))}
        ...
        <TabsContent value="Security">
          <div className="max-w-2xl space-y-5">
            ...
            <div className="glass-panel rounded-2xl [&]:border-destructive/30 overflow-hidden">
              ...
              <div className="p-5 flex items-center justify-between gap-4">
                ...
                <Button variant="outline" className="border-destructive/40 text-destructive hover:bg-destructive/10 rounded-xl h-9 px-4 text-sm shrink-0" onClick={() => toast.error("Contact support@getfrugal.dev to delete your account")}>
                  Delete Account
                </Button>
              </div>
            </div>
              ...
              <div className="p-5 space-y-3">
                {[
                  ...
                  <div key={label} className="flex items-center justify-between py-1">
                    ...
                    <Button variant="outline" size="sm" disabled className="rounded-xl h-8 px-3 text-xs border-border/40 shrink-0">
                      Export · Soon
                    </Button>
                  </div>
                ))}
