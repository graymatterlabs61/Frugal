import { baseLayout, infoBox, statRow, divider } from "../../layout.js";
...
export function ticketReceivedHtml(payload: TicketReceivedPayload): string {
  ...
  const sla = slaMap[payload.priority ?? "normal"] ?? "within 24 hours";
  const priorityRaw = payload.priority ?? "normal";
  const priorityLabel = priorityRaw.charAt(0).toUpperCase() + priorityRaw.slice(1);
