  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/projects", {
        ...
        body: JSON.stringify({ name: name.trim(), description: description || undefined, color }),
      });
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map((project) => {
            const colorClass = colorMap[project.color ?? "slate"] ?? colorMap.slate;
            return (
              <div key={project.id} className="relative group">
                <Link
                  href={`/projects/${project.id}`}
                  className="border border-border rounded-2xl p-5 bg-card hover:border-primary/30 hover:bg-primary/2 transition-all duration-150 cursor-pointer block"
                >
